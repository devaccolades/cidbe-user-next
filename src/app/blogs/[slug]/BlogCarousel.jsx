  ...
  return (
    <section className="h-[690px] overflow-hidden  md:h-[768px] lg:h-screen sticky top-0 -mt-[80px] lg:-mt-[95px] -z-50 ">
      <div className="absolute z-10 top-[50%] flex flex-col gap-[20px] left-0 right-0 transform -translate-y-1/2 text-center ">
        <h1 className="font-[clash-display-medium] text-white text-[32px] lg:text-[64px] leading-[39px] w-[90%] custom-blog-innder-title-width md:w-[40%] mx-auto lg:leading-[78px]  capitalize text-center">
          {blogDetails?.title}
        </h1>
        <div className="flex flex-row gap-[10px] mx-auto w-full justify-center text-[10px] items-center lg:text-[12px]">
          <p className="py-[4px] flex justify-center items-center px-[10px] bg-white font-[general-sans-semibold] rounded-[6px] text-[--secondary-cl]">
            {blogDetails?.date_added}
          </p>
          <p className="py-[2px] px-[10px] bg-white font-[general-sans-semibold] rounded-[6px] text-[--secondary-cl] flex flex-row justify-center items-center gap-[1px]">
            <Image src={clockIcon} alt="flats in Thrissur" />
            {blogDetails?.time_to_read} Min read
          </p>
        </div>
      </div>
      <Slider {...settings}>
