...
const featuredProjectAddOnDatas = [
  ...
  {
    title: "CIDBI CANDOR: Your Destination for Ready to Occupy Apartments in Thrissur",
    description: `
...
function ProjectListing({ title }) {
  ...
  return (
    <main className="bg-[--primary-cl] bg-cover bg-no-repeat project-list-bg -mt-[80px] lg:-mt-[95px]">
      ...
      {pathname === '/featured-projects' && <div className='containers res-custom-container flex flex-col gap-[6px] lg:gap-[20px] pt-[30px] pb-[30px] lg:pb-[60px]'>
        {featuredProjectAddOnDatas.map((item, index) => (
          <AddOnContent
            item={item}
            key={index}
            isOpend={isOpend}
...
const AddOnContent = ({ item, isOpend, setOpen, index }) => (
  <>
    <div className="flex flex-col">
      ...
      >
        <h2 className="text-[16px] lg:text-[26px] font-[clash-display-medium]">{item?.title}</h2>
        <motion.div
          animate={{
