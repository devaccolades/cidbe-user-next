...
const EnquiryModal = dynamic(() => import("../EnquiryForm/EnquiryModal"), {
  ssr: false,
...
function Brochure({ data }) {
  ...
  const overViewIcon = [
    ...
    { icon: fourth, title: "Status", description: data?.status },
    {
      icon: fifth,
      title: "Area Range",
      description: `${data?.area_from || ""} - ${data?.area_to || ""} Sq.Ft`,
    },
  ];
        ...
        <div className="flex flex-col md:flex-row gap-[20px] lg:gap-[30px]">
          ...
          <div className="flex flex-col mb-[20px] md:mb-0 md:w-[53%] lg:w-[70%]">
            ...
            <div className="mt-auto flex flex-row gap-[10px]">
              {data?.brochure && (
                <button
                  onClick={() => getBrochure(data?.id)}
                  className="mt-[20px] py-[10px] px-[18px] text-[--secondary-cl] border border-[--secondary-cl] text-[#ffff] text-[15px] inline-flex items-center gap-[8px] rounded-[6px] lg:inline-flex hidden"
                >
              ...
              <button
                onClick={handleOpen}
                className="mt-[20px] py-[10px] px-[18px] bg-[#052D23] text-[#ffff] text-[15px] inline-flex items-center gap-[8px] rounded-[6px] lg:inline-flex hidden"
              >
                <span>Enquire Now</span>
                <Image
                  src={arrow_outwardicon}
          ...
          <div className="md:w-[47%] lg:w-[30%] bg-[#BFD8BD] rounded-[12px] p-[30px] flex flex-col right-side-width">
                ...
                {overViewIcon.map((item, index) => (
                  <div key={index} className="flex items-center mb-[15px]">
                    <div className="md:w-[30px] md:h-[30px] lg:w-auto lg:h-auto flex-shrink-0">
                      <Image
                        src={item.icon}
                        alt="Premium  4 Bhk apartments in thrissur"
                        width={30}
                        height={30}
                        className="md:w-[30px] md:h-[30px] lg:w-auto lg:h-auto"
                      />
                    </div>
            ...
            <div className="mt-auto flex flex-col gap-[10px]">
              {data?.brochure && (
                <button
                  onClick={() => getBrochure(data?.id)}
                  className="mt-[20px] py-[10px] px-[18px] border border-[--secondary-cl] text-[--secondary-cl] text-[15px] flex items-center justify-center gap-[8px] rounded-[6px] w-full lg:hidden"
                >
                  <span>Download Brochure</span>
                  <Image
                    src={downloadIcon}
                    alt="Premium  3 Bhk apartments in thrissur"
                    width={16}
                    height={16}
                  />
                </button>
              ...
              <button
                onClick={handleOpen}
                className="py-[10px] px-[18px] bg-[#052D23] text-[#ffff] text-[15px] flex items-center justify-center gap-[8px] rounded-[6px] w-full lg:hidden"
              >
                <span>Enquire Now</span>
                <Image
                  src={arrow_outwardicon}
